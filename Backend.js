...
app.post('/cars', (req, res) => {
    ...
    if (!make || !model || !year || !color || !owner) return res.status(400).json({ error: "please fill out all fields correctly."});

    if (isNaN(year) || year > 2999 || year < 1900)
    {
        res.redirect('/RR/RustBadTable.html')
    ...
    db.run(
        'INSERT INTO cars (make, model, year, color, owner) VALUES (?, ?, ?, ?, ?)',
           ...
           function(err) {
               ...
               res.redirect('/RR/RustSuccess.html');
           }
    );
    }
});
...
app.get('/cars/:req', (req, res) => {
    const q = req.params.req;

    db.all('SELECT * FROM cars WHERE id = ? OR make LIKE ? OR model LIKE ? OR year LIKE ? OR color LIKE ? OR owner LIKE ?', [q, q, q, q, q, q], (err, rows) => {
        if (err) {
